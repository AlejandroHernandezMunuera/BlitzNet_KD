...
def zoomout_tutor(image, gt_bboxes, params):
    X_out = tf.random_uniform([], 1.05, params['X_out'])
    h, w, ch = tf.unstack(tf.to_float(tf.shape(image)))
    zoomout_color = params['zoomout_color']+ [0] + [0] + [0]*params['segmentation_tutor_softoutput_dim']
...
def scale_distortions_tutor(image, gt_bboxes, gt_cats, conf, params):
    """Samples a random box according to overlapping
    with gt objects criteria and crops it from an image"""
    if np.random.uniform(0, 1.0) < args.zoomout_prob:
        image, gt_bboxes = zoomout_tutor(image, gt_bboxes, params)
    ...
    gt_bboxes = tf.boolean_mask(gt_bboxes, mask)
    gt_cats = tf.boolean_mask(gt_cats, mask)
    conf = tf.boolean_mask(conf, mask)
    w = tf.to_float(x2-x1)
    h = tf.to_float(y2-y1)
    ...
    return cropped_image, gt_bboxes, gt_cats, conf
...
def filter_small_gt_tutor(gt_bboxes, gt_cats, conf, min_size):
    mask = tf.logical_and(gt_bboxes[:, 2] >= min_size,
                          gt_bboxes[:, 3] >= min_size)
    return tf.boolean_mask(gt_bboxes, mask), tf.boolean_mask(gt_cats, mask), tf.boolean_mask(conf, mask)
...
def data_augmentation_tutor(img, gt_bboxes, gt_cats, tt_bboxes, tt_cats, tt_conf, gt_seg, aug_seg, tt_seg, config):
    ...
    img = tf.concat([img, tf.cast(gt_seg, tf.float32), tf.cast(aug_seg, tf.float32), tf.cast(tt_seg, tf.float32)], axis=-1)
    all_boxes = tf.concat( [ gt_bboxes, tt_bboxes ], axis = 0 )
    all_cats = tf.concat( [ gt_cats, tt_cats ], axis = 0 )
    all_conf = tf.concat( [ tf.ones(tf.shape(gt_cats), dtype=tf.float32), tt_conf ], axis = 0 )

    img, all_boxes, all_cats, all_conf = scale_distortions_tutor(img, all_boxes, all_cats, all_conf, params)
    img, all_boxes = mirror_distortions(img, all_boxes, params)
    
    # XXX reference implementation also randomizes interpolation method
    img_size = config['image_size']
    img_out = tf.image.resize_images(img[..., :3], [img_size, img_size])
    all_boxes, all_cats, all_conf = filter_small_gt_tutor(all_boxes, all_cats, all_conf, 2/config['image_size'])
    
    seg_shape = config['fm_sizes'][0]
    
    gt_seg = tf.expand_dims(tf.expand_dims(img[..., 3], 0), -1)
    gt_seg = tf.squeeze(tf.image.resize_nearest_neighbor(gt_seg, [seg_shape, seg_shape]))
    gt_seg = tf.cast(tf.round(gt_seg), tf.int64)
    
    aug_seg = tf.expand_dims(tf.expand_dims(img[..., 4], 0), -1)
    aug_seg = tf.squeeze(tf.image.resize_nearest_neighbor(aug_seg, [seg_shape, seg_shape]))
    aug_seg = tf.cast(tf.round(aug_seg), tf.int64)
    
    tt_seg = tf.expand_dims(img[..., 5:], 0)
    tt_seg = tf.squeeze(tf.image.resize_nearest_neighbor(tt_seg, [seg_shape, seg_shape]))
    
    return img_out, all_boxes, all_cats, all_conf, gt_seg, aug_seg, tt_seg
